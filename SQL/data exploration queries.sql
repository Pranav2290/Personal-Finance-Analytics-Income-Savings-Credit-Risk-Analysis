/** 10-01-2026
This data set dont have more than one record per user(all unique).
So, we can't evaluate whether a person's year-over-year debt or savings increased or decreased
**/
...
-- * The data set dont have more than one record per user, so we can compare previous values with that record of same user.
